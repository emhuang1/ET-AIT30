...
This Github repository corresponds to the following paper: 
...
“code_tri_sec.R”: For each patient, compute the second-level AI using his/her raw tri-axial smartphone accelerometer data, then uses those results to find the minute-level AI and the day-level AI-T30. Note that the AI-T30 that is produced by this script is unscaled (i.e., it has not been multiplied by 60).
Step 3: AI Minute 
“ai_plots.R”: Plot occurrence of AI minutes over the course of each day in the window of interest, separately for each participant.
